...
function connect() {
  ...
  try {
    currentBot = mineflayer.createBot({
      ...
      physicsEnabled: false, // Tetap false
  ...
  currentBot.once('spawn', () => {
    ...
    // Perbaikan: Paksa bot berhenti total berkali-kali saat spawn
    currentBot.clearControlStates();
    if (currentBot.physics) currentBot.physics.enabled = false;
  ...
  currentBot.on('end', (reason) => {
    ...
    if (pingInterval) clearInterval(pingInterval);
    currentBot = null;
    setTimeout(connect, 20000); // Dipercepat re-connectnya
  });
